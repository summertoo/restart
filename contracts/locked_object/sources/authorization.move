...
/// 创建授权管理器
public fun create_authorization(owner: address, ctx: &mut TxContext): Authorization {
    Authorization {
        id: object::new(ctx),
        owner,
...
/// 撤销操作员权限
public fun revoke_operator(auth: &mut Authorization, operator: address, ctx: &TxContext) {
    assert!(auth.owner == tx_context::sender(ctx), 0);
