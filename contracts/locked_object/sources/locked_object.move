...
/// 存入代币
public fun deposit<T>(locked_object: &mut LockedObject<T>, payment: Coin<T>, ctx: &mut TxContext) {
...
public fun withdraw<T>(
    ...
    let remaining_daily_limit =
        types::get_max_withdrawal_per_day(&locked_object.lock_rules) - today_withdrawn;
    let fee = utils::basis_points_to_value(
        amount,
        types::get_withdrawal_fee_rate(&locked_object.lock_rules),
    );
    ...
    // 检查锁仓时间
    if (
        time_elapsed < types::get_min_lock_period(&locked_object.lock_rules) && !types::get_emergency_withdrawal(&locked_object.lock_rules)
    ) {
